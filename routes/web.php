...
$GLOBALS['ROUTES'] = [
    ...
    // Auth
    ['POST', '~^/api/auth/register$~', function () { Auth::register(); }],
    ['POST', '~^/api/auth/login$~',    function () { Auth::login(); }],
    ['POST', '~^/api/auth/logout$~',   function () { Auth::logout(); }],

    // OAuth stubs
    ['GET',  '~^/api/oauth/google$~',   function () { Auth::oauth_google_start(); }],
    ['GET',  '~^/api/oauth/linkedin$~', function () { Auth::oauth_linkedin_start(); }],
    ...
    ['POST', '~^/api/jobs/apply$~', function () {
        $user = auth_required();
        if ($user['role'] !== 'seeker') return json_response(['error'=>'Forbidden'], 403);
        $in = input_json();
        foreach (['job_id'] as $k) if (!isset($in[$k])) return json_response(['error'=>"Missing $k"], 422);
    ...
    ['POST', '~^/api/jobs/create$~', function () {
        $user = auth_required();
        if ($user['role'] !== 'employer') return json_response(['error'=>'Forbidden'], 403);
        $in = input_json();
        foreach (['title','description','location','category'] as $k) if (!isset($in[$k])) return json_response(['error'=>"Missing $k"], 422);
